...
| 装置 | 设计功能 | 主要硬件 |
|---|---|---|
| 阿波（水面） | 采集加速度、角速度和水压，在本地存储原始数据；预留 LED 提示接口，咬钩检测尚未实物验证 | STM32G031、LSM6DSO、LPS28DFW、W25Q256、LiPo 供电/充电与 LED 驱动 |
| 智能水中（钩侧） | 通过受力梁和应变全桥采集钓线张力，作为回收后离线分析的辅助标签；也记录运动和压力 | 共用 MCU、IMU、压力传感器及 Flash，另装 316L 弹性梁、应变片全桥与 NAU7802 |

“共板”指两块设备使用同一 PCB 设计、分别选择不同贴装器件；图中阿波和水中件并非同一块板。两端计划独立记录，回收后导出数据，不依赖水中的实时无线链路。以上均为设计功能，尚未经过实物钓组验证。
